...
function create(initialState) {
	...
	const httpLink = new HttpLink({
		uri: process.env.NEWSELFWALES_GRAPHQL_URL, // Your GraphQL endpoint
	});
	...
	if (!process.browser) {
		...
		const subscriptionClient = new SubscriptionClient(
			process.env.NEWSELFWALES_GRAPHQL_SUBSCRIPTIONS_URL,
			{
				reconnect: true,
				// connectionParams: {
	...
	return new ApolloClient({
		...
		link,
		// link: new HttpLink({
		// 	uri: process.env.NEWSELFWALES_GRAPHQL_URL, // Server URL (must be absolute)
